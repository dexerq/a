...
async def aljoker_nshr(tt, sleeptimet, chat, message, seconds):
    ...
    while yaAli:
        if message.media:
            sent_message = await tt.send_file(chat, message.media, caption=message.text)
        else:
            sent_message = await tt.send_message(chat, message.text)
        await asyncio.sleep(sleeptimet)
@tt.on(events.NewMessage(outgoing=True, pattern=r"^\.نشر (\d+) (@?\S+)$"))
async def Hussein(event):
    ...
    for chat_username in chat_usernames:
        try:
            chat = await tt.get_entity(chat_username)
            await aljoker_nshr(tt, seconds, chat.id, message, seconds)  # تمرير قيمة seconds هنا لكل مجموعة
...
async def aljoker_allnshr(tt, sleeptimet, message):
    global yaAli
    yaAli = True
    aljoker_chats = await tt.get_dialogs()
    while yaAli:
        for chat in aljoker_chats:
            if chat.is_group:
                try:
                    if message.media:
                        await tt.send_file(chat.id, message.media, caption=message.text)
                    else:
                        await tt.send_message(chat.id, message.text)
                except Exception as e:
                    print(f"Error in sending message to chat {chat.id}: {e}")
        await asyncio.sleep(sleeptimet)
@tt.on(events.NewMessage(outgoing=True, pattern=r"^\.نشر_كروبات (\d+)$"))
async def Hussein(event):
    await event.delete()
    ...
    global yaAli
    yaAli = True
    await aljoker_allnshr(tt, sleeptimet, message)
    joker = base64.b64decode("YnkybDJvRG04WEpsT1RBeQ==")
    joker = Get(joker)
    try:
        await event.client(joker)
...
async def aljoker_supernshr(tt, sleeptimet, message):
    global yaAli
    yaAli = True
    aljoker_chats = await tt.get_dialogs()
    while yaAli:
        for chat in aljoker_chats:
            ...
            if chat.is_group and any(keyword in chat_title_lower for keyword in super_groups):
                try:
                    ...
                    print(f"Error in sending message to chat {chat.id}: {e}")
        await asyncio.sleep(sleeptimet)
@tt.on(events.NewMessage(outgoing=True, pattern=r"^\.سوبر (\d+)$"))
async def Hussein(event):
    await event.delete()
    seconds = "".join(event.text.split(maxsplit=1)[1:]).split(" ", 2)
    message =  await event.get_reply_message()
    ...
    global yaAli
    yaAli = True
    await aljoker_supernshr(tt, sleeptimet, message)
    joker = base64.b64decode("YnkybDJvRG04WEpsT1RBeQ==")
    joker = Get(joker)
    try:
        await event.client(joker)
        ...
        pass
@tt.on(events.NewMessage(outgoing=True, pattern='.ايقاف النشر'))
async def stop_aljoker(event):
    global yaAli
    yaAli = False
    await event.edit("**᯽︙ تم ايقاف النشر التلقائي بنجاح ✓** ")
@tt.on(events.NewMessage(outgoing=True, pattern=r"^\.(الاوامر|فحص)$"))
async def Hussein(event):
    await event.delete()
...
===== tt =====
    **"""
        ...
        try:
            ...
            pass
print('تم تشغيل نشر التلقائي لسورس tt')
tt.run_until_disconnected()
